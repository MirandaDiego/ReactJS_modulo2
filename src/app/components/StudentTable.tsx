...
export const StudentTable = ({ students }: Props) => {
    return (
        <table className="w-full border border-gray-600 rounded-md overflow-hidden">
            ...
            <tbody>
                {students.map(item => (
                    <tr key={item.id} className="text-gray-800 bg bg-gray-400 border-b border-gray-600">
                        <td className="p-3 flex items-center">
                            <img src={item.avatar} alt={item.name}  className="w-14 h-14 rounded-full mr-3"/>
                            <div>
                        ...
                        <td className="p-3">
                            {item.active &&  <div className="px-2 py-1 inline-block border border-green-800 bg-green-600 text-white text-xs">Active</div>}
                            {!item.active &&  <div className="px-2 py-1 inline-block border border-red-800 bg-red-600 text-white text-xs">Inactive</div>}
                        </td>
